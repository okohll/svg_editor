...
public class SVGVisualResourceResourceModifier {
	...
	public static int showPropertyModifierDialog(SVGVisualResourceObject resourceObject) {
		...
		if (resourceObject != null) {
			...
			// getting all the widgets linked with the attributes of the
			// resource object
	...
	/**
	 * returns a widget allowing to modify the children of the given resource
	 * object
	 * 
	 * @param resourceObject
	 *            a resource object
	 * @return a widget allowing to modify the children of the given resource
	 *         object
	 */
	protected static SVGVisualResourceWidget getChildWidget(SVGVisualResourceObject resourceObject) {
		...
		if (resourceObject.getChildren().size() > 0) {
			...
			LinkedList children = resourceObject.getChildren();
			if (children.size() > 0) {
				child = (SVGVisualResourceObjectChild) children.getFirst();
				...
				if (type != null) {
					...
					if (type.equals("colorchooser")) {
						...
						widget = new SVGVisualResourceGradientColorChooserChildWidget(
								resourceObject);
					}
				}
			}
	 ...
	 */
	protected static SVGVisualResourceWidget getAttributeWidget(
		...
		if (resourceObjectAttribute != null && resourceObjectAttribute.getModel() != null) {
			...
			if (type != null) {
				...
				if (type.equals("combo")) {
					...
					widget = new SVGVisualResourceValidatedEntryAttributeWidget(
					...
					widget = new SVGVisualResourceVectorCircleChooserAttributeWidget(
							resourceObjectAttribute);
					...
					widget = new SVGVisualResourcePatternSizeChooserAttributeWidget(
							resourceObjectAttribute);

				} else if (type.equals("editablecombo")) {

					widget = new SVGVisualResourceEditableComboAttributeWidget(
							resourceObjectAttribute);
				}
			}
		}
