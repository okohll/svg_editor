...
public class SelectionModule extends ModuleAdapter {
	...
	protected Icon notesIcon, notesDisabledIcon;

	/**
	 * the menu item that is displayed in the menu bar to handle regular
	 * selections
	 */
	...
	/**
	 * the menu item that is displayed in the menu bar to handle all selection
	 * actions
	 */
	...
	/**
	 * the menu item that is displayed in the menu bar to handle enter group
	 * actions
	 */
	protected JMenuItem enterGroupMenuItem;

	/**
	 * the menu item that is displayed in the menu bar to handle exit group
	 * actions
	 */
	...
	protected JMenuItem notesMenuItem;
	...
	protected void createMenuAndToolItems() {
		...
		notesLabel = ResourcesManager.bundle.getString(notesId + "ItemLabel");
		...
		// creating the listener to the all selection and deselection menu and tool
		// items
		...
		notesIcon = ResourcesManager.getIcon(notesId, false);
		notesDisabledIcon = ResourcesManager.getIcon(notesId, true);
		...
		allSelectionMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A, java.awt.Toolkit
		...
		allDeselectionMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_D, java.awt.Toolkit
				.getDefaultToolkit().getMenuShortcutKeyMask(), false));
		...
		enterGroupMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E, java.awt.Toolkit
				.getDefaultToolkit().getMenuShortcutKeyMask(), false));
		...
		lockMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_L, java.awt.Toolkit
				.getDefaultToolkit().getMenuShortcutKeyMask(), false));
		...
		// creating the notes menu item
		notesMenuItem = new JMenuItem(notesLabel, notesIcon);
		notesMenuItem.setDisabledIcon(notesDisabledIcon);
		notesMenuItem.addActionListener(lockActionListener);
		//notesMenuItem.setAccelerator(KeyStroke.getKeyStroke("ctrl shift N"));
		notesMenuItem.setEnabled(false);
		...
		svgHandleManager.addHandlesListener(new HandlesListener() {
			...
			public void handleChanged(SVGHandle currentHandle, Set<SVGHandle> handles) {
				...
				if (lastHandle != null && selectionChangedListener != null
						&& lastHandle.getSelection() != null) {

					// if a selection listener is already registered on a selection
				...
				if (currentHandle != null) {
					...
					manageSelection(new HashSet<Element>(currentHandle.getSelection().getSelectedElements()));
	...
	/**
	 * updates the state of the enter and exit group action menu items and the
	...
	protected void updateMenuItems() {
		...
		if (handle != null) {
			...
			if (selectedElements.size() == 1) {
				...
				if (selectedElement != null && Selection.isParentElementEligible(selectedElement)) {
	...
	public HashMap<String, JMenuItem> getMenuItems() {
		...
		menuItems.put(notesId, notesMenuItem);
	...
	public Collection<PopupItem> getPopupItems() {
		...
		PopupItem groupEnterItem = new PopupItem(Editor.getEditor(), enterGroupId, enterGroupLabel,
				enterGroupId) {
		...
		PopupItem groupExitItem = new PopupItem(Editor.getEditor(), exitGroupId, exitGroupLabel,
				exitGroupId) {
			...
			public JMenuItem getPopupItem(LinkedList<Element> nodes) {
				...
				if (handle != null) {
					...
					if (parentElement != null
							&& !parentElement.equals(handle.getCanvas().getDocument().getDocumentElement())) {
		...
		PopupItem lockItem = new PopupItem(Editor.getEditor(), lockId, lockLabel, lockId) {
		...
		PopupItem unLockItem = new PopupItem(Editor.getEditor(), unLockId, unLockLabel, unLockId) {
		...
		// Oliver: creating the notes popup item
		PopupItem notesItem = new PopupItem(Editor.getEditor(), notesId, notesLabel, notesId) {
				...
				if (handle != null) {
					...
					if (selectedElements.size() > 0 && selection.isSelectionLocked()) {
						...
						menuItem.addActionListener(new ActionListener() {
							...
							public void actionPerformed(ActionEvent e) {
								// Oliver: TODO: notes edit action here
								//selection.unLock();
							}
						});
					}
