...
public abstract class ExportDialog extends JDialog {
	...
	/**
	 * the set of the runnables that will be used to initialize the dialog each
	 * time it is shown
	 */
	...
	public int showExportDialog(Document document) {
		...
		if (document != null) {
			...
			int x = (int) (Editor.getParent().getLocationOnScreen().getX()
					+ Editor.getParent().getWidth() / 2 - getWidth() / 2), y = (int) (Editor
					.getParent().getLocationOnScreen().getY()
					+ Editor.getParent().getHeight() / 2 - getHeight() / 2);
			...
			// waits until the parameters are chosen and one of the buttons is
			// clicked
			// TODO: This seems a wierd thing to do, is this necessary?
			while (isVisible()) {
