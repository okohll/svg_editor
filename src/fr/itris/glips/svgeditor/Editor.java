...
public class Editor {
	...
	public Editor() {
		/*
		 * try { //PlasticXPLookAndFeel laf=new PlasticXPLookAndFeel();
		 * //PlasticXPLookAndFeel.set3DEnabled(true); WindowsLookAndFeel laf = new
		 * WindowsLookAndFeel(); UIManager.setLookAndFeel(laf); } catch
	 ...
	 *          whether the quit action is disabled
	 * @param exitFromJVM
	 *          whether the JVM will be exited when the user requires to exit from
	 *          the editor
	...
	public void init(Container parent, String fileToBeLoaded, final boolean displayFrame,
		...
		if (parent instanceof JFrame) {
			...
			contentPane.add(moduleManager.getToolBarManager().getToolsBar(), BorderLayout.WEST);
			...
			final Rectangle frameBounds = new Rectangle(screenInsets.left, screenInsets.top,
					screenSize.width - screenInsets.left - screenInsets.right, screenSize.height
			...
			applet.getContentPane().add(moduleManager.getToolBarManager().getToolsBar(),
					BorderLayout.NORTH);
	...
	protected Map<String, Rectangle> getWidgetBoundsMap() {
		...
		if (doc != null) {
			...
			if (root != null) {
				...
				// getting the node containing the nodes giving the bounds of the
	...
	/**
	 * @return whether the JVM will be exited when the user requires to exit from
	 *         the editor
	 */
