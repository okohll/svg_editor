...
/**
 * the superclass of all the segment classes that defines segments in a svg path
 * 
...
public abstract class Segment {
	...
	/**
	 * the last control point of the segment if the segment corresponds to a
	 * Bezier curve
	 */
	protected Point2D controlPoint;

	/**
	 * the other control point of the segment if the segment corresponds to a
	 * Bezier curve
	 */
	 ...
	 *            the segment that lies just before this segment
	 */
	public Segment(Segment previousSegment) {

		this.previousSegment = previousSegment;
		this.segmentIndex = (previousSegment != null ? previousSegment.getSegmentIndex() + 1 : 0);
	}
	...
	public boolean indexMatchSegment(int index) {
		...
		return ((index >= segmentIndex * 10) && (index < (segmentIndex + 1) * 10));
	}
	 ...
	 * 
	 * @param previousSegment
	 *            the previous segment before this one
	 */
	...
	/**
	 * @return the last control point of the segment if the segment corresponds
	 *         to a Bezier curve
	 */
	...
	/**
	 * @return the other control point of the segment if the segment corresponds
	 *         to a Bezier curve
	 */
	...
	/**
	 * returns whether the segment defined by the segment string representation
	 * corresponds to a relative command
	 * 
	 * @param segmentString
	 *            the string representation of a segment
	 * @return whether the segment defined by the segment string representation
	 *         corresponds to a relative command
	 */
		...
		if (numberStrings != null && numberStrings.length > 0) {
			...
			for (String str : numberStrings) {
				...
				try {
					...
					System.err.println("Error parsing " + str + " into a double. " + ex);
					dVal = Double.NaN;
				}
	...
	protected java.util.List<Point2D> getPoints(String pointsString) {
		...
		if (coordinatesArray != null && coordinatesArray.length > 0
			...
			for (int i = 0; i < coordinatesArray.length; i += 2) {
				...
				pointsList.add(new Point2D.Double(coordinatesArray[i], coordinatesArray[i + 1]));
			}
		}
	 ...
	 *            a relative point
	 * @param basePoint
	 *            the base point to which the relative point is relative to
	...
	/**
	 * computes the symetrical point of the provided original point, relatively
	 * to the center point
	 * 
	 * @param originalPoint
	 *            the original point
	...
	/**
	 * computes the symetrical point of the provided original point, relatively
	 * to the center point of the segment formed by the provided points
	 * 
	 * @param originalPoint
	 *            the original point
	 * @param pt1
	 ...
	 * @return computes the symetrical point of the provided original point,
	 *         relatively to the center point of the segment formed by the
	 *         provided points
	 */
	public static Point2D computeSymetricRelCenter(Point2D originalPoint, Point2D pt1, Point2D pt2) {
	 ...
	 * 
	 * @param previousSegment
	 *            the previous segment before the one that will be created
	 * @param segmentString
	 *            the string describing the segment to create
