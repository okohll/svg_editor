...
public class SVGColorsManager {
	 ...
	 */
	private static LinkedHashMap<String, SVGW3CColor> w3cColorsMap = new LinkedHashMap<String, SVGW3CColor>();
	...
	static {
		...
		try {
			...
			doc = docBuild.parse(SVGColorsManager.class.getResourceAsStream("xml/svgColors.xml"));
		} catch (IOException | SAXException | ParserConfigurationException ex) {
			System.err.println("Error parsing XML file. " + ex);
		}
	...
	/**
	 * retrieves all the colors in the given svg document and fills the list and
	 * the map of the colors
	 * 
	 * @param svgColorsDocument
	 *            the document containing the name of each and its associated
	 *            rgb value
	 */
	protected static void retrieveW3CColors(Document svgColorsDocument) {
		...
		if (svgColorsDocument != null && svgColorsDocument.getDocumentElement() != null) {
			...
			// for each svg colors, gets the name and the value and adds them to
			// the list
			for (Node cur = svgColorsDocument.getDocumentElement().getFirstChild(); cur != null; cur = cur
					.getNextSibling()) {
		...
		if (color == null) {
			...
			if (color == null && colorString.length() == 7) {
				...
				try {
					...
					System.err.println("Error decoding RGB colour: " + ex);
				}
	...
	public static String getColorString(Color color) {
		...
		String sr = Integer.toHexString(color.getRed()), sg = Integer.toHexString(color.getGreen()), sb = Integer
