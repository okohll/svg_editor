...
public class ToolBarPanel extends JPanel{
	...
	public void initialize(Element element) {
		...
		try {
			checkBox.setSelected(Boolean.parseBoolean(
					jwidgetEdition.getProperty(element, 
							jwidgetEdition.getPropertiesList().get(30))));
	...
	protected void build() {
		...
		try {
			...
			displayScrollBarLabel=jwidgetEdition.getBundle().getString("displayToolBar");
		}catch (Exception ex) {ex.printStackTrace();}
		...
		checkBoxActionListener=new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				jwidgetEdition.setProperty(
						currentElement, jwidgetEdition.getPropertiesList().get(30), 
